...
extern char *yytext;
extern MessageFormat mrsk_compile_error_message_format[];
extern MessageFormat mrsk_runtime_error_message_format[];
...
static void format_message(MessageFormat *format, VString *v, va_list ap)
{
    ...
    for (i=0; format->format[i]!='\0'; i++) {
        if (format->format[i] != '$') {
            mrsk_vstr_append_character(v, format->format[i]);
            continue;
        }
        ...
        switch (cur_arg.type) {
            ...
            case STRING_MESSAGE_ARGUMENT:
                ...
                mrsk_vstr_append_string(v, cur_arg.u.string_val);
                break;
            case POINTER_MESSAGE_ARGUMENT:
...
void self_check()
{
    if (strcmp(mrsk_compile_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("compile error message format error.\n"));
    }
    if (strcmp(mrsk_compile_error_message_format
    ...
    if (strcmp(mrsk_runtime_error_message_format[0].format, "dummy") != 0) {
        DBG_panic(("runtime error message format error.\n"));
    }
    if (strcmp(mrsk_runtime_error_message_format
...
void mrsk_compile_error(CompileError id, ...)
{
    ...
    self_check();
    va_start(ap, id);
    line_number = mrsk_get_current_interpreter()->current_line_number;
    mrsk_vstr_clear(&message);
    format_message(&mrsk_compile_error_message_format[id], &message, ap);
...
void mrsk_runtime_error(int line_number, RuntimeError id, ...)
{
    ...
    va_start(ap, id);
    mrsk_vstr_clear(&message);
    format_message(&mrsk_runtime_error_message_format[id],
                   &message, ap);
    fprintf(stderr, "%3d:%s\n", line_number, message.string);
    va_end(ap);
...
int yyerror(char const *str)
{
    ...
    mrsk_compile_error(PARSE_ERR, STRING_MESSAGE_ARGUMENT, "token", near_token,
